...
전체 관리자만 탑승 관리 간사님 명단을 전용 Google Sheet에 동기화할 수 있는 Edge Function입니다.
`전체 명단` 탭과 각 호차별 탭을 갱신하고, 탑승 미확인·출발 후 탑승 미확인·미탑승·비고 있음 행에 색상을 적용합니다.
탑승 관리 간사님 페이지를 전체 관리자가 열어 둔 동안 명단 변경 후 5초 뒤 자동 호출되며, 화면의 동기화 버튼으로 즉시 호출할 수도 있습니다.
