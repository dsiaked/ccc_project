...
async function run() {
  const email = `test_cascade_${Math.floor(Math.random() * 1000000)}@example.com`;
  ...
  try {
    ...
    const { data: inquiry, error: createError } = await supabase.rpc('create_personal_inquiry', {
      p_category: 'etc',
      p_title: 'Test Inquiry for Cascade Delete',
      p_content: 'This is a test inquiry to verify cascade delete.'
    });
    ...
    console.log('Adding a message/reply to the inquiry...');
    const { data: message, error: messageError } = await supabase.rpc('add_personal_inquiry_message', {
      p_inquiry_id: inquiry.id,
      p_message: 'This is a reply message'
    });
