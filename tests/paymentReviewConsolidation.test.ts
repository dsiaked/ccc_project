...
test('the consolidated payment review defaults to every campus and offers a review-needed filter', () => {
  ...
  assert.match(reviewPage, /<option value="transfer-unconfirmed">본부 확인 필요<\/option>/);
...
test('payment review clearly identifies the all-paid state', () => {
  assert.match(
    reviewPage,
    /const hasNoUnpaidPeople =[\s\S]*!errorMessage[\s\S]*review\.unpaidReservations\.length === 0/
  );
  ...
  assert.match(reviewPage, /현재 확인할 개인 미입금자가 없습니다/);
});
